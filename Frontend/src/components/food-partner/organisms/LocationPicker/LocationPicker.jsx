...
import { FALLBACK, PIN_HTML } from '../../utils/locationPicker';
...
export function LocationPicker({ lat, lng, search, onSearch, onPlace }) {
  ...
  return (
    <>
      <Box className="ob-map">
        <Box ref={mapEl} className="ob-map__canvas" />

        <Box className="ob-map__ui">
          <Box className="ob-map__search">
          ...
          <Box className="ob-map__zoom">
            <PlainButton type="button" onClick={() => map.current?.zoomIn()} aria-label="Zoom in">
              <Icon name="plus" className="ob-ico" />
            </PlainButton>
            <PlainButton type="button" onClick={() => map.current?.zoomOut()} aria-label="Zoom out">
              <Icon name="minus" className="ob-ico" />
            </PlainButton>
          </Box>

          <PlainButton type="button" className="ob-map__locate" onClick={useMyLocation}>
            <Icon name="track" className="ob-ico" />
            {locating ? 'Locating…' : 'Use my location'}
          </PlainButton>

          <Box className="ob-map__view">
      ...
      <Box className="ob-grid ob-grid--2">
        <Label className="ob-field">
          <Inline className="ob-label">GPS latitude</Inline>
          <Input type="text" className="ob-input is-read" value={lat} readOnly placeholder="Drop the pin" />
        </Label>
        <Label className="ob-field">
          <Inline className="ob-label">GPS longitude</Inline>
