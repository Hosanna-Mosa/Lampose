...
export function VisitRequestDialog({ listing, sharing, intent, onClose, onVerified }) {
  /*
    ...
    <Box className="vr-overlay" onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}>
      ...
      >
        ...
        <Masthead className="vr-head">
          ...
          <Inline className="vr-head__badge"><Icon name="calendar" className="exp-ico" /></Inline>
          <Box>
            <Heading level={2} className="vr-title" id="vr-title">
        ...
        {step === 'known' ? (
          <Form className="vr-form" onSubmit={submitKnown} noValidate>
            <Box className="vr-known">
              <Inline className="vr-known__label">Requesting as</Inline>
              <Strong className="vr-known__name">{known.name}</Strong>
              <Inline className="vr-known__phone">{maskPhone(known.phone)}</Inline>
            </Box>

            <Label className="vr-consent">
              <Input type="checkbox" checked={form.consent} onChange={set('consent')} />
              <Inline>Send me the owner&rsquo;s answer on WhatsApp. You&rsquo;ll see it on this page either way.</Inline>
            </Label>

            {err && <Text className="vr-err" role="alert">{err.message}</Text>}

            <PlainButton className="vr-submit" type="submit" disabled={Boolean(busy)} aria-busy={Boolean(busy)} ref={askButton}>
              {busy || 'Ask the owner'}
            </PlainButton>
          ...
          <Form className="vr-form" onSubmit={submitForm} noValidate>
            ...
            <Label className="vr-field">
              ...
              <Box className="vr-phone">
                <Emphasis>+91</Emphasis>
                <Input
            ...
            <PlainButton className="vr-submit" type="submit" disabled={Boolean(busy)} aria-busy={Boolean(busy)}>
          ...
          <Form className="vr-form" onSubmit={submitOtp} noValidate>
            ...
            {err && (
              <Text className="vr-err" role="alert">
                {err.message}
                {err.attemptsLeft === 0 && (
                  <PlainButton type="button" className="vr-linkbtn" onClick={onClose}>Start again</PlainButton>
                )}
              </Text>
            )}
            ...
            {closed ? (
              ...
              <PlainButton className="vr-submit" type="button" onClick={onClose}>
                Browse other rooms
              </PlainButton>
            ) : (
              <PlainButton
            ...
            {!closed && (
              <Box className="vr-resend">
                {cooldown > 0
                  ? <Inline>Didn&rsquo;t get it? You can resend in {cooldown}s.</Inline>
                  : (
                    <PlainButton type="button" className="vr-linkbtn" onClick={resend} disabled={Boolean(busy)}>
                      Resend the code
                    </PlainButton>
                  )}
                <PlainButton type="button" className="vr-linkbtn" onClick={() => { setStep('form'); setErr(null); }}>
                  Change number
                </PlainButton>
              </Box>
            )}
