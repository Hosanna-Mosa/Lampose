import React from 'react';

/* <dd> — passes every attribute straight through and adds nothing of its
   own, so the rendered markup is identical to the hand-written element. The
   ref is forwarded: code that held a ref to the raw element still holds one. */
export const DescriptionDetail = React.forwardRef(({ children, ...rest }, ref) => (
  <dd ref={ref} {...rest}>{children}</dd>
));
DescriptionDetail.displayName = 'DescriptionDetail';
