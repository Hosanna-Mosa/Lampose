...
import { Block, Box, Field, FilePick, Note, StepFrame, Tappable, TextField } from "@/components/common";
import { ProductForm, emptyItem } from "@/components/common";
import { Btn, Card, Chip, Icon, IconBtn, ModalSheet, Seg, Text } from "@/components/common";
        ...
        <Block glyph="menu" title="Categories & items" subtitle={COPY.manualCategoryHelp}>
          {data.menuCategories.length === 0 ? (
            <Box style={styles.empty}>
              ...
              <Box style={styles.chipWrap}>
                {MENU_CATEGORY_SUGGESTIONS.map((s) => (
                  <Tappable key={s} accessibilityRole="button" onPress={() => addCategory(s)} style={styles.suggest}>
            ...
            data.menuCategories.map((category) => (
              <Box key={category.id} style={styles.category}>
                <Box style={{ flexDirection: "row", alignItems: "center", gap: space[2] }}>
      ...
      >
        ...
        <Box style={styles.chipWrap}>
          {MENU_CATEGORY_SUGGESTIONS.filter((s) => !categoryNames.includes(s)).map((s) => (
            <Tappable key={s} accessibilityRole="button" onPress={() => setNewName(s)} style={styles.suggest}>
              <Text variant="title3" color="brand">
                {s}
              </Text>
            </Tappable>
