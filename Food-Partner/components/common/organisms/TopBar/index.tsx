/* ══════════════════════════════════════════════════════════════════════════
   ...
   ScrollView gets a header that scrolls away, which is the bug this component
   exists to make impossible — nothing here reads a scroll offset, so there is
   no way to "sticky" it after the fact.
   ...
   The title is LEFT-aligned, never centred. A centred title shares the row
   with a back control and an action, so it truncates at about half the width
   — and this app's titles are restaurant names and step counters, which are
...
export function TopBar({
  ...
  /**
   * Where back goes, as a destination name. Read only by screen readers — the
   * control itself is a chevron, so the row keeps its width for the title.
...
const styles = StyleSheet.create({
  ...
  content: { height: HEADER_HEIGHT, flexDirection: "row", alignItems: "center", paddingRight: space[2] },
  backBtn: { width: touch.min, height: touch.min, alignItems: "center", justifyContent: "center", borderRadius: 999 },
