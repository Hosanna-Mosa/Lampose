 ...
 *
 * Ported from `Stay Partner/lib/log.ts` / `User App/lib/log.ts`.
 *
 * ## Why these calls are not just left as `console.warn`
 *
 * A few of them log the *error object* a fetch or a socket connect failed
 * with. On a handset nobody is reading that console, but it is still a copy
 * of this restaurant's own data written somewhere it did not ask to go, and
 * on Android it is readable by anything holding the log permission.
 *
 * Nothing here changes what the partner sees. Every call site this replaces
