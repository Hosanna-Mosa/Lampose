...
import { Box, Heading, Image, Inline, Link, PlainButton, Text } from '../../../common/atoms';

export function PropertyDetailModal({ property, onClose, onDelete }) {
  ...
  return (
    <Box style={{
      ...
      <Box className="glass-card modal-content" style={{
        maxWidth: '720px',
        width: '100%',
        maxHeight: '92vh',
        overflowY: 'auto',
        ...
        <Box style={{ position: 'relative', height: '220px' }}>
          ...
          <Box style={{ position: 'absolute', bottom: '16px', left: '20px', right: '20px' }}>
            <Box style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
              <Inline className={`badge ${badgeClass}`}>
                {category}
              </Inline>
              <Inline style={{ fontSize: '0.75rem', fontWeight: 700, padding: '3px 10px', borderRadius: '12px', background: '#D8993E', color: '#ffffff' }}>
                {stayType}
              </Inline>
            </Box>
            <Heading level={2} style={{ fontSize: 'clamp(1.3rem, 4vw, 1.8rem)', fontWeight: 800, color: '#ffffff', lineHeight: '1.2' }}>
              {name}
            </Heading>
            <Box style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-sub)', fontSize: '0.85rem', marginTop: '4px' }}>
        ...
        <Box style={{ padding: '20px' }}>
          ...
          <Box style={{
            ...
            {dailyPrice > 0 && (
              <Box>
                <Box style={{ display: 'flex', alignItems: 'center', gap: '4px', color: '#f7c784', fontSize: '0.78rem', fontWeight: 600 }}>
                  <Clock size={14} />
                  <Inline>Short Stay (1-7 Days)</Inline>
                </Box>
                <Box style={{ fontSize: '1.4rem', fontWeight: 800, color: '#ffffff', marginTop: '2px' }}>
                  ₹{dailyPrice} <Inline style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>/ day</Inline>
                </Box>
                <Inline style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Option: {shortStayDuration}</Inline>
              </Box>
            )}

            {monthlyPrice > 0 && (
              <Box>
                <Box style={{ display: 'flex', alignItems: 'center', gap: '4px', color: '#ffffff', fontSize: '0.78rem', fontWeight: 600 }}>
                  <Calendar size={14} color="#D8993E" />
                  <Inline>Long Stay (1+ Month)</Inline>
                </Box>
                <Box style={{ fontSize: '1.4rem', fontWeight: 800, color: '#D8993E', marginTop: '2px' }}>
                  ₹{monthlyPrice} <Inline style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>/ month</Inline>
                </Box>
                <Inline style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Option: {longStayDuration}</Inline>
              </Box>
            )}

            <Box>
              <Inline style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>Security Deposit</Inline>
              <Box style={{ fontSize: '1.1rem', fontWeight: 700, color: '#ffffff', marginTop: '4px' }}>
                ₹{deposit || 0}
              </Box>
            </Box>
          ...
          <Box style={{
            ...
            <Box>
              <Inline style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Owner Name:</Inline>
              <Box style={{ fontSize: '1rem', fontWeight: 700, color: '#ffffff', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <User size={16} color="#D8993E" />
                <Inline>{ownerName}</Inline>
              </Box>
            </Box>
            <Box>
              <Inline style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Contact:</Inline>
              <Box style={{ fontSize: '1rem', fontWeight: 700, color: '#D8993E' }}>
                {ownerMobile}
              </Box>
            </Box>
            {address && (
              <Box style={{ width: '100%', paddingTop: '8px', borderTop: '1px solid rgba(255, 255, 255, 0.06)' }}>
                <Inline style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Street Address:</Inline>
                <Text style={{ fontSize: '0.85rem', color: 'var(--text-sub)', marginTop: '2px' }}>{address}</Text>
              </Box>
            )}
          </Box>
          ...
          <Box style={{ marginBottom: '20px' }}>
            <Heading level={4} style={{ fontSize: '1.05rem', color: '#ffffff', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <ShieldCheck size={18} color="#D8993E" />
              <Inline>{category} Category Parameters</Inline>
            </Heading>
          ...
          {amenities.length > 0 && (
            <Box style={{ marginBottom: '24px' }}>
              <Heading level={4} style={{ fontSize: '1.05rem', color: '#ffffff', marginBottom: '10px' }}>
                Included Amenities
              </Heading>
          ...
          <Box style={{
            ...
            >
              <Trash2 size={16} />
              <Inline>Delete Listing</Inline>
            </PlainButton>

            <PlainButton onClick={onClose} className="btn btn-primary" style={{ padding: '10px 24px', background: '#D8993E' }}>
              Close Window
            </PlainButton>
          </Box>
        </Box>
