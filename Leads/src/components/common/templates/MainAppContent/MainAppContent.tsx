...
import { LiveProgressModal } from '../../organisms/LiveProgressModal';
import { NewlyExtractedModal } from '../../organisms/NewlyExtractedModal';
import { userApi, User } from '../../../../api/userApi';
import { Loader2 } from 'lucide-react';
import { Box, Main, Text } from '../../atoms';
...
export function MainAppContent() {
  ...
  if (authLoading) {
    return (
      <Box className="min-h-screen bg-slate-50 flex items-center justify-center text-slate-900">
        <Box className="text-center space-y-3">
          <Loader2 className="w-10 h-10 text-cyan-600 animate-spin mx-auto" />
          <Text className="text-xs text-slate-500 font-semibold">Validating session...</Text>
        </Box>
      </Box>
    );
  }
  ...
  return (
    <Box className="min-h-screen bg-slate-50 text-slate-900 flex flex-col">
      {/* Top Header */}
