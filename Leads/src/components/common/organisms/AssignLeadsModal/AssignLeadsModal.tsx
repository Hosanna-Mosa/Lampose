...
import { Box, Heading, Inline, Label, Option, PlainButton, Select, Text } from '../../atoms';
...
export const AssignLeadsModal: React.FC<AssignLeadsModalProps> = ({
  ...
  return (
    <Box className="fixed inset-0 bg-slate-900/40 backdrop-blur-md flex items-center justify-center z-50 p-4">
      <Box className="bg-white border border-slate-200 rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-6 relative">
        <PlainButton
          onClick={onClose}
        ...
        <Box className="flex items-center gap-3">
          <Box className="w-10 h-10 rounded-2xl bg-cyan-50 border border-cyan-200 text-cyan-600 flex items-center justify-center">
            <UserCheck className="w-5 h-5" />
          </Box>
          <Box>
            <Heading level={3} className="text-base font-bold text-slate-900">Assign Leads to Employee</Heading>
            <Text className="text-xs text-slate-500">Assign {leadIds.length} lead(s) to a team member</Text>
          </Box>
        </Box>
        ...
        <Box className="flex items-center justify-end gap-3 pt-2">
          ...
          >
            ...
            <Inline>Confirm Assignment</Inline>
          </PlainButton>
        </Box>
      </Box>
    </Box>
