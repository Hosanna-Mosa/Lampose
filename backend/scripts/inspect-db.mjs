/* One-off reconnaissance against the onboarding database.
   Prints collection names, document counts and the key shape of a sample
   document per collection — never the connection string.

     node scripts/inspect-db.mjs            (from backend/)
*/
import 'dotenv/config';
import mongoose from 'mongoose';

const URI = process.env.MONGO_URI;
if (!URI) { console.error('MONGO_URI missing from backend/.env'); process.exit(1); }

const shapeOf = (v, depth = 0) => {
  if (v === null) return 'null';
  if (Array.isArray(v)) {
    return v.length ? `[${shapeOf(v[0], depth + 1)}] x${v.length}` : '[]';
  }
