/* ══════════════════════════════════════════════════════════════════════════
   ...
   The crosshair on the address parts of the sign-up form and the profile.
   Deliberately SEPARATE from `hooks/useDriverLocation.ts`, which watches the
   rider continuously and feeds the dispatcher: that hook is a subscription
   whose whole job is to keep reporting, and this is a single measurement taken
   because somebody pressed a button. Sharing them would mean either a form
   that starts a watcher or a watcher that reverse-geocodes on every fix.
...
export async function locateMe(): Promise<LocatedAddress> {
  ...
  try {
    ...
    throw new LocationRefused(
      "We could not get a fix. Move somewhere more open, or type your address.",
    );
  }
