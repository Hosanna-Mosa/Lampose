/* ══════════════════════════════════════════════════════════════════════════
   Create the "restaurant approved" WhatsApp template in Twilio.

     npm run twilio:restaurant-template                 show what WOULD be created
     npm run twilio:restaurant-template -- --create     create it in Twilio
     npm run twilio:restaurant-template -- --create --submit
                                                        create it AND send it to Meta
     npm run twilio:restaurant-template -- --submit-sid HXxxxx
                                                        send an existing one to Meta
     npm run twilio:restaurant-template -- --status HXxxxx
                                                        has Meta approved it yet?
     npm run twilio:restaurant-template -- --list       what already exists
     npm run twilio:restaurant-template -- --delete HXxxxx    remove one

   This is the message an owner is sent when an administrator approves their
   restaurant: the account is live, and here is how to sign in to it. The
   definition — body, samples, name, category — is in
   `infrastructure/twilio/restaurantApprovedTemplate.js`, including WHY the
   password is in the body and what to do if Meta refuses it for that.
   ...
   Put the id in `.env` and restart the backend:
     ...
     TWILIO_RESTAURANT_APPROVED_CONTENT_SID=HXxxxx

   Until Meta approves it, a template can only be sent to a number that has
   messaged the Lampose sender in the last 24 hours — which an owner being
   approved has almost certainly not done. `--status` says when it is through.
   ══════════════════════════════════════════════════════════════════════════ */
require('dotenv').config();

const {
  FRIENDLY_NAME, LANGUAGE, CATEGORY, BODY, SAMPLES, render,
} = require('../src/infrastructure/twilio/restaurantApprovedTemplate');

const ENV_KEY = 'TWILIO_RESTAURANT_APPROVED_CONTENT_SID';
...
const preview = () => {
  ...
  console.log('\n  ── Rendered, as the owner reads it ───────────────');
  render().split('\n').forEach((line) => console.log(`  │ ${line}`));
  console.log('');
};
...
(async () => {
  ...
  if (statusSid) {
    ...
    if (found.status === 'approved') {
      ...
      console.log('  Rejected. Delete it (--delete), fix the wording in restaurantApprovedTemplate.js, and create it again.\n');
    } else {
      console.log(`  Not submitted to Meta yet. To submit: npm run twilio:restaurant-template -- --submit-sid ${statusSid}\n`);
    }
    return;
  }
  ...
  if (submitSid) {
    ...
    console.log(`  Check it with: npm run twilio:restaurant-template -- --status ${submitSid}\n`);
    return;
  }
  ...
  /* Refuse to make a second copy of a template that already exists — Twilio
     ...
     `lampose_restaurant_approved`s is one where nobody knows which id the server
     is actually using. */
  ...
  if (already) {
    ...
    console.log(`  Its approval: npm run twilio:restaurant-template -- --status ${already.sid}\n`);
    return;
  }
  ...
  if (!has('--submit')) {
    ...
    console.log(`  To submit: npm run twilio:restaurant-template -- --submit-sid ${sid}\n`);
    return;
  }
  ...
  console.log('  Approval usually takes minutes to a few hours.\n');
  console.log(`  Check it:   npm run twilio:restaurant-template -- --status ${sid}`);
